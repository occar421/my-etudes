# Rust API client for openfga_client
...
Put the package under your project folder in a directory named `openfga_client` and add the following to `Cargo.toml` under `[dependencies]`:

```
openfga_client = { path = "./openfga_client" }
```
